...
- Prefer simple composition first; use custom BufferGeometry sparingly. BUT when the reference shows a
  printed label, brand text, logo, or surface pattern, a procedurally-generated DataTexture is REQUIRED
  (see "Procedural surface texture, labels & text" below) — do not leave such a surface blank.
...
- Prefer a few well-placed, surface-attached motifs over many detached blobs.
  For raised/relief ornament use the geometry decals above; for FLAT printed
  graphics (labels, text, logos, photographic patterns) use a procedural
  DataTexture map instead (see next section) — never leave them out.

Procedural surface texture, labels & text (REQUIRED when the reference shows them):
- A printed label, brand text, logo, decorative print, or photographic surface
  pattern is part of the object's IDENTITY. A clean but blank surface loses.
  When the reference shows any of these, apply a procedurally-GENERATED
  `THREE.DataTexture` as the material `map` (and `emissiveMap` for backlit
  graphics). Do NOT leave the surface a flat color and do NOT spell text with geometry.
- Technique (cheap; you COMPUTE the pixels in code, so it does NOT count against
  the 50 KB literal budget): allocate `const W=256,H=256,d=new Uint8Array(W*H*4)`;
  fill the base color (sampled from the photo); then draw with plain loops
  (deterministic — NO Math.random; use Math.sin or a seeded LCG):
    · LABEL → lighter rounded rectangle band + 2-5 darker horizontal bars to read
      as text lines + a colored logo block.
    · BRAND TEXT / LOGO → blocky glyph rectangles or a colored emblem patch in the
      right color and position.
    · PATTERN (galaxy/marble/floral/speckle) → base gradient + procedural noise/specks/rings.
    · STRIPES/CHECK → modulo bands.
  Then `const t=new THREE.DataTexture(d,W,H,THREE.RGBAFormat); t.colorSpace=THREE.SRGBColorSpace;
  t.needsUpdate=true; mat.map=t;`. On lathe/cylinder bodies set `t.wrapS=THREE.RepeatWrapping`
  and offset/repeat so the label sits on the front at the right height. Data <= 1 MB (256x256x4 is fine).
- Color accuracy: sample the dominant + accent colors and use exact hex. Avoid surfaces
  that render too dark — for matte/plastic keep roughness ~0.4-0.7 and a mid-bright base;
  cap metalness <= 0.5 for colored plastics/ceramics so the body does not darken.
- Texture is ADDITIVE: it goes on TOP of correct geometry, it is NOT a substitute for
  shape, relief, or pattern. Keep an ornamented/patterned body's silhouette and relief
  AND add the texture map; never replace a patterned body with a plain darker solid.
- Identity completeness: never omit a jar LID, can LABEL, bottle CAP, teapot HANDLE+SPOUT,
  watering-can SPOUT, or the floral/galaxy PRINT — add these before polishing geometry.
...
# R9 "diverse ensemble" augmentation.
#
# This is the verbatim content of harness/material_v3.txt: guidance that pushes
# bright/polished metals toward a moderate EMISSIVE self-light so they read as
# light-toned (this renderer is dim and has no env map, so metals otherwise look
# too dark vs the reference photo).
#
# It is APPENDED to CODER_SYSTEM_PROMPT for roughly HALF of the ensemble
# candidates (see multigen_first_iter / SceneCoderAgent.code emissive flag). The
# other half use the unmodified CODER_SYSTEM_PROMPT — wertox's base prompt is
# never altered for the non-emissive half. wertox's built-in Qwen judge bracket
# then keeps a bright-metal candidate only when it actually wins, so this only
# adds variety to the candidate pool, it never overrides selection.
EMISSIVE_METAL_BLOCK = """
============================================================
METAL BRIGHTNESS via EMISSIVE (this renderer is dim + has no env map, so metals look too dark vs the bright reference photo)
============================================================
Bright/polished metals (aluminum, steel, silver, chrome, gold, brass, copper) MUST look bright, not dark gray.
Use MeshStandardMaterial with a moderate EMISSIVE self-light so the metal matches the reference brightness:
   color:    <light metal hex>          // aluminum/steel/silver '#d6dadf', gold '#e8c84a', brass '#d4b24a', copper '#d08a5a'
   metalness: 0.25
   roughness: 0.32
   emissive:  <same light metal hex>
   emissiveIntensity: 0.40              // lifts dark-gray -> bright lit metal; do NOT exceed 0.6 (avoids a glowing look)
RULES:
- NEVER metalness > 0.4 for metals (no env map -> renders black).
- A shiny metal in the reference must render LIGHT-TONED here; if it looks dark gray, raise emissiveIntensity toward 0.5.
- Non-metals: no emissive; matte (metalness 0, roughness 0.7) with true color.
"""


def coder_system_prompt(emissive: bool = False) -> str:
    """Return the coder system prompt for one ensemble candidate.

    `emissive=False` returns wertox's base CODER_SYSTEM_PROMPT byte-for-byte.
    `emissive=True` appends EMISSIVE_METAL_BLOCK (harness/material_v3.txt) so the
    candidate biases bright/polished metals toward an emissive self-light.
    """
    if not emissive:
