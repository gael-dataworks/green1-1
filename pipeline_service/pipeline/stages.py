...
# Per-submission candidate-pool offset. Added to the validator-injected round
# seed so that distinct submissions (different hotkeys) draw DISJOINT candidate
# seeds and therefore produce DISTINCT outputs — never identical across hotkeys
# (the source-audit duplicate check fails identical outputs). Each submission
# bakes a different default here; the CDN generator overrides it via env so its
# pool matches this repo's regenerated pool at qualification time.
SEED_OFFSET = int(os.environ.get("SN_SEED_OFFSET", "0"))
...
async def multigen_first_iter(
    ...
    *,
    ...
    send_image = (osd is None) or coder_multimodal
    K = ensemble_size
    base_seed = task.seed + SEED_OFFSET

    logger.info(
        f"[MULTIGEN] {task.stem} K={K} | temperature={ensemble_temperature} | "
        f"multimodal={send_image} | osd={'yes' if osd else 'no'} | "
        f"diverse=emissive-half"
    )

    async def _gen(k: int) -> Candidate:
        cand = Candidate(k=k, seed=base_seed + k)
        # R9 "diverse ensemble": roughly HALF the candidates (odd index) append
        # the emissive-metal block (harness/material_v3.txt) to the coder system
        # prompt to bias bright/polished metals toward an emissive self-light;
        # the other half (even index) use wertox's unmodified CODER_SYSTEM_PROMPT.
        # wertox's Qwen judge bracket below keeps a bright-metal candidate only
        # when it actually wins, so this just adds variety to the pool.
        emissive = (k % 2 == 1)
        t0 = time.time()
        try:
    ...
    # R10 (Qwen-only): selection uses ONLY wertox's original single-elimination
    # pairwise Qwen judge bracket. No external GLM selection.
